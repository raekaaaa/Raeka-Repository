...
line_number = 0
print(f"Line {line_number}: {jsondata[str(line_number)]}")

for line_number, line_text in jsondata.items():
    print(f"Line {line_number}: {line_text}")

json_data['0'] = '<p><b>Joy:</b> (narrates) Do you ever look at someone and wonder, "What is going on inside their head?"</p>'
...
for line_number, line_text in jsondata.items():
    script = (f"Line {line_number}: {line_text}")
...
def people(f,r,c):
    ...
    if 1000*f + 600*r + 750*c <= 18000 and 500*f + 750*r + 300*c <= 12000 and 50*f + 45*r + 75*c <=1500 and f >= 0 and r >= 0 and c >= 0:
...
def Collatz_winner(number_list):
    ...
    for number in number_list:
        collatz_data = Collatz(number)
        length = collatz_data["collatz_length"]
        max_collatz = collatz_data["max_collatz"]
        
        if length > max_length or (length == max_length and max_collatz > max_value):
            max_length = length
            max_collatz = max_value
            winner = number
    
    return winner


Collatz_winner([5, 10, 15, 20])
